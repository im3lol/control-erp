...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'فشل في تحميل أوامر البيع' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invoiceTaxPercent = taxPercent || 0
    const calculatedTaxAmount = Math.round(subtotal * invoiceTaxPercent / 100 * 100) / 100
    const totalAmount = subtotal - invoiceDiscountAmount + calculatedTaxAmount
    ...
    // Generate order number: SO-{year}-{seq}
    ...
    const number = generateDocNumber('SO', year, seq)
